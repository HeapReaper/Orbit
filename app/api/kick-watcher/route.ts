...
export async function GET(req: NextRequest) {
  ...
  try {
    const data = await prisma.kick_watcher.findUnique({
      where: { guild_id },
    });
    ...
    console.error(err);
    return NextResponse.json({ error: "Failed to fetch Kick watcher" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  const { guild_id, channel, users, enabled } = data;

  try {
    const updated = await prisma.kick_watcher.upsert({
      where: { guild_id },
      update: {
        channel,
        enabled,
    ...
    console.error(err);
    return NextResponse.json({ error: "Failed to save Kick watcher" }, { status: 500 });
  }
}