...
export async function GET(req: NextRequest) {
  ...
  try {
    const data = await prisma.giveaway_settings.findFirst({ where: { guild_id } });
...
export async function POST(req: NextRequest) {
  ...
  const { guild_id, channel, enabled } = data;

  try {
    const updated = await prisma.giveaway_settings.upsert({
      where: { guild_id },
      update: { channel, enabled },
      create: { guild_id, channel, enabled },
    });
